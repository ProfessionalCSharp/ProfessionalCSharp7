...
namespace BooksServiceSample.Controllers
{
    ...
    public class BookChaptersController : Controller
    {
        ...
        [HttpGet]
        public IEnumerable<Chapter> GetBookChapters() => _bookChaptersService.GetAll();

        // GET api/bookchapters/guid
        [HttpGet("{id}", Name = nameof(GetBookChapterById))]
        public IActionResult GetBookChapterById(Guid id)
        {
            Chapter chapter = _bookChaptersService.Find(id);
            if (chapter == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        public IActionResult PostBookChapter([FromBody]Chapter chapter)
        {
            if (chapter == null)
            {
                return BadRequest();
            ...
            return CreatedAtRoute(nameof(GetBookChapterById), new { id = chapter.ChapterId }, chapter);
        }

        // PUT api/bookchapters/guid
        [HttpPut("{id}")]
        public IActionResult PutBookChapter(Guid id, [FromBody]Chapter chapter)
        {
